...
def gauss_hermite_1d(m, M=None):
    ...
    relative accuracy for the tiny outer weights (they bottom out around 1e-60
    when the true value is e.g. 1e-350), which destroys any expression in which
    the weight is later multiplied by something exponentially large.  We use the
    ...
    v = np.zeros(m)         # is ~exp(-x^2/2) < 1e-300, so dropping them is exact
    w = np.zeros(m)         # to double precision
...
def quad_coefficients(f, s, m, M, return_nevals=False):
    ...
    (each alpha_i <= m-1, so f*hhat_alpha is integrated by a rule exact on
...
# exact variance decomposition (diagnostics)
